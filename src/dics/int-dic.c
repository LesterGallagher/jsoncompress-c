...
int_dic_t *int_dic_new(unsigned int initialSize) {
    ...
    int_dic_t *dic = (int_dic_t*)malloc(sizeof(int_dic_t));
    dic->data = (int_dic_kvp_t*) calloc(initialSize, sizeof(int_dic_kvp_t));
    dic->table_size = initialSize;
    dic->size = 0;
    return dic;
}
 ...
 */
unsigned int hashmap_hash_int(int_dic_t * m, char* keystring){
...
int hashmap_hash(int_dic_t *m, char* key){
	int curr;
	int i;
...
void hashmap_rehash(int_dic_t *m){
	int i;
	int old_size;
	int_dic_kvp_t* curr;

	/* Setup the new elements */
	int_dic_kvp_t* temp = (int_dic_kvp_t *)
		calloc(2 * m->table_size, sizeof(int_dic_kvp_t));
	...
	for(i = 0; i < old_size; i++){
		...
		int_dic_put(m, curr[i].key, curr[i].value);
	}
	free(curr);
}
