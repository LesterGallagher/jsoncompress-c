...
void push_node(xml_node_t* parent, xml_node_t* child) {
    if (parent->children_len >= INITIAL_ITEM_LEN
        && parent->children_len && !(parent->children_len & (parent->children_len - 1))) { // power of 2
        parent->children = realloc(parent->children, sizeof(xml_val_t) * parent->children_len * 2);
    }
...
xml_node_t *xml_parse(char* xml) {
    xml_node_t document;
    xml_node_t *curr = &document;
    curr->children = malloc(sizeof(xml_val_t));
    curr->children_len = 0;
    curr->parent = NULL;
    ...
    for(int i = 0; i < len; i++) {
        switch(xml[i]) {
            ...
            if (xml[i+1] == '/') {
                // trim excess
                curr->children = realloc(curr->children, sizeof(xml_val_t) * curr->children_len);
                curr = curr->parent;
                while(xml[++i] != '>') {}
                break;
            }
            // start = i+1;
            // while(xml[i+1] != '>') {i++;}
            child = malloc(sizeof(xml_node_t));
            ...
            if (xml[i] == '/') { // self closing
                ...
                child->children = malloc(sizeof(xml_val_t) * INITIAL_ITEM_LEN);
                while(xml[i] != '>' && xml[i] != '/') {
                    i++;
                    while(isspace(xml[i])) {i++;} // skip spaces
                    start = i;
                    ...
                    if (i == start) continue;
                    xml_node_t *attr = malloc(sizeof(xml_node_t));
                    ...
                    if (xml[i] == '=' && xml[i+1] == '"') {
                        ...
                        attr->children_len = 1;
                        attr->children = malloc(sizeof(xml_val_t));
            ...
            child->children = malloc(sizeof(xml_val_t));
